...
if __name__ == "__main__":
    ...
    if not st.session_state.auth:
        ...
        # st.markdown(
        #     'Answers with <img src="https://emoji.slack-edge.com/T024FJS4M/couchbase/4a361e948b15ed91.png" width="20"/> are generated using *RAG* while <img src="https://static-00.iconduck.com/assets.00/openai-icon-253x256-bamv50yy.png" width="20"/> are generated by pure *LLM (ChatGPT)*',
        #     unsafe_allow_html=True,
        # )
        st.markdown(
            'Below you can enter questions and we will process your question twice to showcase running without and with Retrieval Augmented Generation (*RAG*) while you chat with your PDF(s).\n\nAnswers with <img src="https://uxwing.com/wp-content/themes/uxwing/download/brands-and-social-media/google-gemini-icon.png" width="20"/> are generated by pure *LLM (Gemini)* while <img src="https://emoji.slack-edge.com/T024FJS4M/couchbase/4a361e948b15ed91.png" width="20"/> are generated using *RAG* (vector query against Couchbase) and then passing that context with the question to the *LLM*',
            unsafe_allow_html=True,
        )
        ...
        with st.sidebar:
            ...
            st.markdown(
                ...
                * one using pure LLM - Gemini (<img src="https://uxwing.com/wp-content/themes/uxwing/download/brands-and-social-media/google-gemini-icon.png" width="20"/>).
                * one using RAG (<img src="https://emoji.slack-edge.com/T024FJS4M/couchbase/4a361e948b15ed91.png" width="20"/>)
                """,
                unsafe_allow_html=True,
            )
            ...
            if st.checkbox("View Code"):
                st.write(
                    "View the code here: [Github](https://github.com/marcobevilacqua94/easy-webrag-langchain-demo/blob/main/chat_with_pdf.py)"
                )
        ...
        if question := st.chat_input("Ask a question based on the PDF(s)"):
            ...
            rag_context = {"context": retriever.get_relevant_documents(str(question)), "question": str(question)}
        ...
        # Add hyperlink to view context
        if st.session_state.show_rag_button:
            if st.button("What we sent to the Couchbase/Gemini LLM via RAG"):
